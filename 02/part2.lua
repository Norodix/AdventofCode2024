...
while line ~= nil do
	...
	if check_safe(nums) then
		...
		local limited_safe = false
		for i, _ in ipairs(nums) do
			-- quick and dirty deep copy
			local limited_nums = { table.unpack(nums) }
			table.remove(limited_nums, i)
			limited_safe = limited_safe or check_safe(limited_nums)
		end
		safes = safes + (limited_safe and 1 or 0)
	end
